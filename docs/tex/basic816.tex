...
struct {
    ...
    Only two commands are implemented to support it: \keyword{DIR} to print a directory listing,
    and \keyword{LOAD} to load a BASIC program.
...
NEXT        .fill 4  ; Address of the next file header in the RAM disk
    ...
    Included with the source code for the BASIC interpreter is a Python script, \keyword{mkvfs.py} which
    ...
    Reads the BASIC program with the given \param{filename} as if you had typed it manually.
    ...
    The monitor can be summoned using the \keyword{MONITOR} command, but it will also be
    activated if a machine language program uses the \keyword{BRK} instruction.
    ...
    \begin{itemize}
        \item \verb+A <address> <assembly>+---Assemble a line of 65816 assembly code\
        given a starting address.

        \item \verb+C <address1> <address2> [<length>]+---Compare two blocks of memory,
        and lists addresses that differ. The third argument \param{length} is optional,
        and is the number of bytes in the blocks to compare. If no \param{length} is provided,
        only one byte will be compared at the two addresses.

        \item \verb+D <start> [<end>]+---Attempt to disassemble the memory starting at
        the address \param{start} and continuing until the address \param{end} (if no
        \param{end} address is provided, it will attempt to disassemble 256 bytes).

        \item \verb+F <start> <end> <byte>+---Fill a block of memory with a value.
        The block of memory is specified by the \param{start} and \param{end} addresses,
        and the value to store is specified by the \param{byte} argument.

        \item \verb+G [address]+---Start executing the code at the provided \param{address}.
            The \verb+G+ command starts executing the code with a \keyword{JSL} instruction,
            so the code can return to the monitor with either the \keyword{RTL} or the
            \keyword{BRK} instruction.

        \item \verb+J [address]+---Start executing the code at the provided \param{address}.
            The \verb+J+ command starts executing the code with a \keyword{JML} instruction,
            so the code can return to the monitor only with the \keyword{BRK} instruction. 

        \item \verb+H <start> <end> <byte> [byte]...+---Hunt through a block of memory for
            a sequence of \param{byte}s. The block of memory is specified byte the addresses
            \param{start} and \param{end}. Up to eight \param{byte}s may be provided, and
            a matching address will be printed only if that exact sequence of bytes starts
            at that address.

        \item \verb+M <start> [end]+---Display the values stored in the memory block specified
            the addresses \param{start} and \param{end}. If no \param{end} is provided,
        ...
        \item \verb+T <start> <end> <destination>+---``Transfer'' or copy a block of data
            ...
            The source of the data to copy is the block of memory specified by \param{start}
            and \param{end}.
            The destination block starts at \param{destination}.
        ...
        \item \verb+X+---Attempt to return to the program that invoked the monitor. This will
            ...
            program used a \keyword{BRK} instruction. This command will not work if the stack
            has been modified too much, or if the calling program is not written assuming a BRK
            can return (and leaves a byte of ``data'' immediately after the \keyword{BRK} itself).

        \item \verb+> <start> <byte> [byte]...+---Write \param{byte}s of data to a block of memory.
            The block starts at the address \param{start}.
            Up to eight \param{byte}s of data can be provided to the command.
    ...
    If thge \verb+M+ flag is set, an immediate \keyword{LDA} instruction will take a single
    byte operand, but if \verb+M+ is clear, the instruction requires a two byte operand.
    Similarly, the \verb+X+ flag will control the length of \keyword{LDX} and \keyword{LDY}
    instructions.
    ...
    watch for \keyword{REP} and \keyword{SEP} instructions, and track changes to the flags to
